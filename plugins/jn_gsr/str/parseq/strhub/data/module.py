...
class SceneTextDataModule(pl.LightningDataModule):
    ...
    def get_transform(img_size: Tuple[int], augment: bool = False, rotation: int = 0):
        ...
        if augment:
            import os
            if os.environ.get("JN_ENVELOPE", "0") == "1":
                # measured jersey envelope (Table 3 / Appendix A); flips off inside it.
                # Option A single-stage; for Option B the envelope is baked into the
                # materialized crops, so training there runs with JN_ENVELOPE unset.
                from .augment_envelope import build_envelope_transform
                _seed = os.environ.get("JN_ENVELOPE_SEED")
                transforms.append(build_envelope_transform(int(_seed) if _seed else None))
            else:
                from .augment import rand_augment_transform      # stock PARSeq default
